...
@router.get("/predictions/{series_id}", response_model=List[BasePredictions])
async def get_predictions(series_id: str) -> List[BasePredictions]:
    with get_db() as session:
        try:
            series = (
                session.query(Predictions)
                .filter(Predictions.series_id == series_id)
                .all()
            )
            if series:
                return series
